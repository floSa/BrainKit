...
Il n'y a rien d'autre à comprendre. Pas de hiérarchie parallèle, pas de dossier d'attente, pas de galaxie. Le dossier porte la valeur de `domaine:` ; le champ `role:` porte ce que la page **est**. Les deux axes ne se recouvrent pas, et c'est ce qui permet à deux pages de nature différente de vivre dans le même dossier sans que rien ne soit ambigu.

---

## 2. Les 6 natures de page

C'est le champ `role:` du frontmatter qui porte la nature d'une page. **Il ne se devine pas depuis le chemin** : deux pages de nature différente et de même domaine vivent dans le même dossier, et un `ls` ne les distingue pas.

| `role:` | Ce que c'est | Rangée par | Protégée |
|---|---|---|---|
| `consigne` | un objet transverse par construction, qui ne dépend d'aucune valeur de l'axe de rangement | son rôle → `Consignes/` | non |
| `directive` | un objet transverse par construction, qui ne dépend d'aucune valeur de l'axe de rangement | son rôle → `Directives/` | non |
| `hub` | la page d'aiguillage d'un dossier — elle n'explique rien, elle oriente | elle **est** le rangement | non |
| `notion` | ce qu'il faut comprendre | `domaine:` | **oui** |
| `sequence` | ce qui départage plusieurs pages d'un même thème | `domaine:` | non |
| `unite` | ce qu'on vient chercher dans le brain | `domaine:` | non |
...
**Personne ne choisit un dossier.** Le chemin se dérive du champ `domaine:` — 8 valeurs de premier niveau, et le validateur le vérifie page par page : une page hors de son dossier dérivé est une violation, pas un rangement personnel.
...
L'axe n'est **pas exclusif** : une page peut porter plusieurs valeurs de `domaine:`. Son dossier est alors celui de la valeur dominante — la valeur de l'axe est celle qui rassemble le plus de la page, et le préfixe `transverse` est réservé aux pages qu'aucune valeur ne rassemble.

| Domaine | Dossier | Portée |
|---|---|---|
| `domaine-a` | `Domaine A/` | le premier domaine |
| `domaine-b` | `Domaine B/` | le deuxième domaine |
| `domaine-c` | `Domaine C/` | le troisième domaine |
| `domaine-d` | `Domaine D/` | le quatrième domaine |
| `domaine-e` | `Domaine E/` | le cinquième domaine |
| `domaine-f` | `Domaine F/` | le sixième domaine |
| `domaine-g` | `Domaine G/` | le septième domaine |
| `transverse` | `Transverse/` | ce qui n'a aucun centre de gravité |

**L'axe transverse `marqueurs:`** traverse l'arbre : un hub par valeur **portée**, dans `Marqueurs/`. Un hub y naît quand une page porte la valeur, pas avant — une page vide dans un graphe est un nœud de plus qui ne rassemble rien.

**L'axe transverse `secteurs:`** traverse l'arbre : un hub par valeur **portée**, dans `Secteurs/`. Un hub y naît quand une page porte la valeur, pas avant — une page vide dans un graphe est un nœud de plus qui ne rassemble rien.

---

## 4. Lire une page `role: unite`

Une unité suit toujours le même gabarit. De haut en bas :
...
| `Ce que c'est` | La SEULE section où la prose est permise. | ce que le haut de page dit déjà |
| `Ce qu'elle fait / Ce qu'elle ne fait pas` | un tableau à deux colonnes : *Fait* et *Ne fait pas* | de la prose |
| `Repères` | 5 étiquette(s) obligatoires : `Repère A`, `Repère B`, `Repère C`, `Repère D`, `Repère E` | de la prose, et aucune étiquette hors de la liste |
...
- `Notes datées` — n'existe que si au moins une entrée datée.
...
|---|---|
| Nature | `nature` |
| Origine et date | `origine`, qualifié par `date_origine` |
| Langue | `langue` |
| Fiabilité | `fiabilite` |
...
Toutes les pages `sequence` sont réunies par le hub de `Séquences/`, où qu'elles vivent dans l'arbre. **C'est le lien retour qui fait la grappe** : un hub qui cite N pages sans qu'aucune le cite ajoute un nœud et ne rassemble rien.
...
2. **Tu hésites entre deux pages** → la page `sequence` du dossier, et directement sa section écrite à la main.
...
4. **Tu pars d'un axe transverse** → `Marqueurs/`, un hub par valeur de `marqueurs:`.
5. **Tu pars d'un axe transverse** → `Secteurs/`, un hub par valeur de `secteurs:`.
...
| `Marqueurs/` | build_mocs, depuis `axes.transverses[marqueurs]` |
| `Secteurs/` | build_mocs, depuis `axes.transverses[secteurs]` |
| `Séquences/Séquences.md` | build_mocs |
| `AI/index/liens.md` | build_links |
| `bandeaux des unités` | build_bandeau |
| `Documentation/domaines/taxonomie.md` | kit, depuis `axes` |
