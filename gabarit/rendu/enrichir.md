...
Le skill **`enrichir-brainref`** porte cette règle. On l'appelle en langage naturel, depuis un agent lancé **dans le dossier du vault**.
...
| P2 | les hubs parents, jusqu'à la racine | remontée de chemin : chaque niveau de `$D` porte son hub | généré (zone AUTO) — **le corps, lui, est à relire** | la page atterrit directement dans le dossier de domaine : P1 et P2 désignent alors la même page, et il faut le DIRE |
| P3 | la séquence du dossier — `role: sequence` | `ls "$D"/'Séquence - '*` | la vue `.base` : **automatique** si elle filtre sur `domaine` — la page y entre toute seule ; le fichier `.base` n'est PAS généré (`vue_embarquee.genere: false`) : il se lit, il ne se réécrit pas · « Ce que la séquence montre » · « Voir aussi » : à écrire | le dossier ne porte aucune page `role: sequence` — et alors la question est : faut-il en créer une ? |
...
| P5 | les pairs — les autres `role: unite` du dossier | `ls "$D"/*.md`, puis lire `role:` dans chaque frontmatter | à écrire, **réciprocité obligatoire** : `contredit:` (symétrique) et la **paire** `prolonge:` / `prolonge_par:` (inverse). Un pair écarté est un CHOIX, pas un oubli : le dire | le dossier ne porte aucune autre page `role: unite` — la page est la première du dossier |
...
| P7 | le hub de ralliement `Séquences/` — **hors du dossier**, à la racine | il ne se lit pas dans `ls "$D"` : il vit à la racine et réunit tous les `role: sequence` du brain | sa zone AUTO est générée ; **le lien retour vers `[[Séquences]]` dans « Voir aussi » de la page : à écrire** — c'est lui qui fait la grappe, pas la liste | cette capture ne crée aucune page `role: sequence` — le hub n'est dans le rayon d'une insertion que si elle en CRÉE une |
| P8 | les hubs de `marqueurs:` — `Marqueurs/` | les valeurs de `marqueurs:` que la page porte | généré — le hub d'une valeur naît quand une page la porte | la page ne porte aucune valeur de `marqueurs:` |
| P9 | les hubs de `secteurs:` — `Secteurs/` | les valeurs de `secteurs:` que la page porte | généré — le hub d'une valeur naît quand une page la porte | la page ne porte aucune valeur de `secteurs:` |
...
- **`domaine:`** — l'axe qui **range** (domaine) : *où ça se range*. C'est lui, et lui seul, qui décide du chemin.
- **`nature:`** — l'axe qui **qualifie** (nature) : *ce que c'est*, 9 valeurs fermées.

`Documentation/domaines/taxonomie.md` porte un **arbre de décision déterministe** : questions **fermées**, en **ordre strict**, la première réponse positive gagne. On le déroule, on ne tranche pas au jugé — et l'ordre des questions **est** la décision de conception, prise une fois, valable pour toutes les pages.
...
| `domaine` | [D] le CHEMIN doit suivre : la page **déménage**, par `git mv` (`chemin_categorie`) · [G] le hub quitté ET le hub d'accueil · [!] l'entrée / la sortie des vues filtrées sur `domaine` · [!] le SEUIL de promotion : la page change de population — un départ peut dépromouvoir un sous-dossier, une arrivée le promouvoir. C'est `brainkit re-seuiller`, pas une capture · [D] la valeur appartient au vocabulaire déclaré (`vocabulaire_ferme`) · [G] l'index — le champ y est publié, donc lu par le skill d'exploitation sans ouvrir la page | `brainkit valider --regle chemin_categorie` |
| `nature` | [D] la valeur appartient au vocabulaire déclaré (`vocabulaire_ferme`) · [D] les champs CONDITIONNELS qu'il commande — `variante:`, `complement:`, `reference:` : les retirer quand la condition cesse de tenir (`gabarit_par_role`) · [G] la zone AUTO du bandeau — une cellule sans source affiche « — », jamais une valeur plausible · [!] les vues `role: sequence` : le champ est une colonne ou le tri — elles se lisent en direct, mais un filtre qui le vise change de membres · [G] l'index — le champ y est publié, donc lu par le skill d'exploitation sans ouvrir la page | `brainkit valider --regle gabarit_par_role` ; `brainkit generer --check` |
| `marqueurs` | [G] les hubs de `Marqueurs/` — un hub par valeur portée : celui qu'on quitte peut tomber à zéro page et disparaître · [G] l'index — le champ y est publié, donc lu par le skill d'exploitation sans ouvrir la page | `brainkit generer --check` |
| `secteurs` | [G] les hubs de `Secteurs/` — un hub par valeur portée : celui qu'on quitte peut tomber à zéro page et disparaître · [G] l'index — le champ y est publié, donc lu par le skill d'exploitation sans ouvrir la page | `brainkit generer --check` |
| `origine` | [G] la zone AUTO du bandeau — une cellule sans source affiche « — », jamais une valeur plausible · [!] les vues `role: sequence` : le champ est une colonne ou le tri — elles se lisent en direct, mais un filtre qui le vise change de membres · [G] l'index — le champ y est publié, donc lu par le skill d'exploitation sans ouvrir la page | `brainkit generer --check` |
| `date_origine` | [G] la zone AUTO du bandeau — une cellule sans source affiche « — », jamais une valeur plausible · [!] les vues `role: sequence` : le champ est une colonne ou le tri — elles se lisent en direct, mais un filtre qui le vise change de membres · [G] l'index — le champ y est publié, donc lu par le skill d'exploitation sans ouvrir la page | `brainkit generer --check` |
...
| `references_cles` | [D] chaque cible doit exister : un lien mort en frontmatter ne se voit pas à la lecture de la page (`liens_resolus`) | `brainkit valider --regle liens_resolus` |
...
**Toute** écriture dans une page du brain se clôt par le skill **`cloturer-brainref`**.
...
- **Aucune valeur d'axe hors du vocabulaire.** Une famille nouvelle se pose dans `Documentation/domaines/taxonomie.md` — donc dans `brain.yml`, qui le génère — pas dans l'arborescence.
