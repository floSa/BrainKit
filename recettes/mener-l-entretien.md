# Recette — mener l'entretien d'initialisation

> **Pour qui** : un agent de code, quel qu'il soit. Cette recette ne suppose
> qu'un système de fichiers et un terminal. Aucun format de skill propriétaire,
> aucune clé d'API.
>
> **Quand** : l'utilisateur veut créer un brain sur un sujet — « je veux un
> second brain sur X », « crée-moi un brain », « initialise un vault BrainKit »,
> « on reprend l'entretien ».
>
> **Ce que ça produit** : un `brain.yml` valide, puis un vault semé, vert aux
> deux validateurs, et **sans une seule page de contenu**.
>
> **Critère de fin** : `uv run brainkit valider --vault <dossier>` rend 0
> violation dure et 0 avertissement, et `uv run brainkit generer --vault
> <dossier>` rend 0 écart.
